...
void SchedulerCheck(Time_t time) {
    ...
    if(checkCount == 10 && !migrating) {
        ...
        for (unsigned i = 0; i < Machine_GetTotal(); i++) {
            if (i == vmInfo.machine_id) continue; // Skip current machine
...
void StateChangeComplete(Time_t time, MachineId_t machine_id) {
    ...
    std::string stateStr;
    switch (currentState) {
        case S0: stateStr = "S0 (Fully On)"; break;
        case S1: stateStr = "S1 (Standby)"; break;
        case S2: stateStr = "S2 (Light Sleep)"; break;
        case S3: stateStr = "S3 (Deep Sleep)"; break;
        case S4: stateStr = "S4 (Hibernation)"; break;
        case S5: stateStr = "S5 (Soft Off)"; break;
        default: stateStr = "Unknown"; break;
    }
    
    LogMessage("StateChangeComplete(): Machine " + to_string(machine_id) + 
              " is now in state " + stateStr, 2);
...
void SLAWarning(Time_t time, TaskId_t task_id) {
    LogMessage("SLAWarning(): Task " + to_string(task_id) + 
              " violated SLA at time " + to_string(time), 1);
    ...
    std::string slaStr;
    switch (slaType) {
        case SLA0: slaStr = "SLA0 (95% within 1.2x expected time)"; break;
        case SLA1: slaStr = "SLA1 (90% within 1.5x expected time)"; break;
        case SLA2: slaStr = "SLA2 (80% within 2.0x expected time)"; break;
        case SLA3: slaStr = "SLA3 (best effort)"; break;
        default: slaStr = "Unknown SLA"; break;
    }
    
    LogMessage("SLAWarning(): Task " + to_string(task_id) + 
              " has " + slaStr + " requirements", 2);
    ...
    if (taskVM != VMId_t(-1)) {
        LogMessage("SLAWarning(): Task " + to_string(task_id) + 
                  ...
                  " on machine " + to_string(taskMachine), 2);
        ...
        if (slaType == SLA0 || slaType == SLA1) {
            ...
            LogMessage("SLAWarning(): Increased priority for task " + 
                      ...
                      to_string(taskMachine) + " to maximum performance", 1);
                ...
                for (MachineId_t machine : scheduler.GetMachines()) {
                    ...
                    if (info.active_tasks < machineInfo.active_tasks / 2) {
                        ...
                        LogMessage("SLAWarning(): Found less loaded machine " + 
                                  to_string(targetMachine) + " for migration", 2);
                        break;
                    }
                }
                ...
                if (targetMachine == MachineId_t(-1)) {
                    for (MachineId_t machine : scheduler.GetMachines()) {
                        ...
                        LogMessage("SLAWarning(): Powering on machine " + 
                                  to_string(machine) + " for VM with SLA-violating task", 1);
    ...
    LogMessage("SLAWarning(): Current SLA violation counts - SLA0: " + 
              ...
              to_string(slaViolations[SLA2]), 3);
}
...
void Scheduler::Init() {
    ...
    for (const auto& pair : machinesByCPU) {
        LogMessage("  CPU " + to_string(pair.first) + ": " + 
                  to_string(pair.second.size()) + " machines", 2);
    }
    ...
    for (const auto& pair : machinesByCPU) {
        ...
        for (unsigned i = 0; i < numVMsToCreate; i++) {
            ...
            LogMessage("Scheduler::Init(): Created VM " + to_string(vm) + 
                      ...
                      " on machine " + to_string(machine), 2);
        }
    }
...
void Scheduler::MigrationComplete(Time_t time, VMId_t vm_id) {
    ...
    LogMessage("Scheduler::MigrationComplete(): VM " + to_string(vm_id) + 
              ...
              " with " + to_string(vmInfo.active_tasks.size()) + " active tasks", 2);
}

void Scheduler::NewTask(Time_t now, TaskId_t task_id) {
    LogMessage("Scheduler::NewTask(): Processing new task " + to_string(task_id) + 
              " at time " + to_string(now), 2);
    ...
    if (target_vm == VMId_t(-1)) {
        ...
        for (MachineId_t machine : activeMachines) {
            ...
            if (info.cpu == required_cpu) {
                ...
                LogMessage("Scheduler::NewTask(): Found active machine " + 
                          to_string(machine) + " with required CPU", 3);
                break;
            }
        }
        ...
        if (target_machine != MachineId_t(-1)) {
            ...
            LogMessage("Scheduler::NewTask(): Attaching VM " + to_string(target_vm) + 
                      " to machine " + to_string(target_machine), 2);
...
void Scheduler::PeriodicCheck(Time_t now) {
    ...
    LogMessage("Scheduler::PeriodicCheck(): System state - " + 
              ...
              to_string(activeMachines.size()) + " active machines", 2);
    ...
    for (MachineId_t machine : machines) {
        ...
        if (info.active_tasks > 0) {
            LogMessage("Scheduler::PeriodicCheck(): Machine " + to_string(machine) + 
                      " utilization: " + to_string(utilization * 100) + "%", 4);
        }
    }
    ...
    if (!migrating && now > 1000000) { // Wait a bit before starting consolidation
        ...
        if (!underutilizedMachines.empty()) {
            ...
            if (sourceInfo.active_vms > 0) {
                ...
                if (vmToMigrate != VMId_t(-1)) {
                    ...
                    for (MachineId_t machine : activeMachines) {
                        if (machine == sourceMachine) continue; // Skip source machine
...
void Scheduler::Shutdown(Time_t now) {
    ...
    for (MachineId_t machine : machines) {
        ...
        LogMessage("  Machine " + to_string(machine) + 
                  ...
                  ", Active VMs=" + to_string(info.active_vms), 2);
    }
